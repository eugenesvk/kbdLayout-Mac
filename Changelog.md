...
## [Unreleased]
  ...
  - __Added__
    ...
    + `en` add <kbd>🕱</kbd>`math𝐛`, mapped to <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`Math`
        * 𝐐𝐖𝐄𝐑𝐓𝐘𝐔𝐈𝐎𝐏𝐀𝐒𝐃𝐅𝐆𝐇𝐉𝐊𝐋𝐙𝐗𝐂𝐕𝐁𝐍𝐌 <kbd>⇧</kbd>
        * 𝐪𝐰𝐞𝐫𝐭𝐲𝐮𝐢𝐨𝐩𝐚𝐬𝐝𝐟𝐠𝐡𝐣𝐤𝐥𝐳𝐱𝐜𝐯𝐛𝐧𝐦
    + `en` add <kbd>🕱</kbd>`math𝑖`, mapped to <kbd>⎇</kbd><kbd>i</kbd> in <kbd>🕱</kbd>`Math`
        * 𝑄𝑊𝐸𝑅𝑇𝑌𝑈𝐼𝑂𝑃𝐴𝑆𝐷𝐹𝐺𝐻𝐽𝐾𝐿𝑍𝑋𝐶𝑉𝐵𝑁𝑀 <kbd>⇧</kbd>
        * 𝑞𝑤𝑒𝑟𝑡𝑦𝑢𝑖𝑜𝑝𝑎𝑠𝑑𝑓𝑔.𝑗𝑘𝑙𝑧𝑥𝑐𝑣𝑏𝑛𝑚
    + `en` add <kbd>🕱</kbd>`math𝐛𝑖`, mapped to <kbd>⎇</kbd><kbd>b</kbd>,<kbd>⎇</kbd><kbd>i</kbd> or <kbd>⎇</kbd><kbd>i</kbd>, <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`Math`
        * 𝑸𝑾𝑬𝑹𝑻𝒀𝑼𝑰𝑶𝑷𝑨𝑺𝑫𝑭𝑮𝑯𝑱𝑲𝑳𝒁𝑿𝑪𝑽𝑩𝑵𝑴 <kbd>⇧</kbd>
        * 𝒒𝒘𝒆𝒓𝒕𝒚𝒖𝒊𝒐𝒑𝒂𝒔𝒅𝒇𝒈𝒉𝒋𝒌𝒍𝒛𝒙𝒄𝒗𝒃𝒏𝒎
    + `en` add <kbd>🕱</kbd>`math𝓈`, mapped to <kbd>⎇</kbd><kbd>s</kbd> in <kbd>🕱</kbd>`Math`
        * 𝒬𝒲..𝒯𝒴𝒰.𝒪𝒫𝒜𝒮𝒟.𝒢.𝒥𝒦.𝒵𝒳𝒞𝒱.𝒩. <kbd>⇧</kbd>
        * 𝓆𝓌.𝓇𝓉𝓎𝓊𝒾.𝓅𝒶𝓈𝒹𝒻.𝒽𝒿𝓀𝓁𝓏𝓍𝒸𝓋𝒷𝓃𝓂
    + `en` add <kbd>🕱</kbd>`math𝓈𝓫`, mapped to <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`math𝓈`
        * 𝓠𝓦𝓔𝓡𝓣𝓨𝓤𝓘𝓞𝓟𝓐𝓢𝓓𝓕𝓖𝓗𝓙𝓚𝓛𝓩𝓧𝓒𝓥𝓑𝓝𝓜 <kbd>⇧</kbd>
        * 𝓺𝔀𝓮𝓻𝓽𝔂𝓾𝓲𝓸𝓹𝓪𝓼𝓭𝓯𝓰𝓱𝓳𝓴𝓵𝔃𝔁𝓬𝓿𝓫𝓷𝓶
    + `en` add <kbd>🕱</kbd>`math𝔣`, mapped to <kbd>⎇</kbd><kbd>f</kbd> in <kbd>🕱</kbd>`Math`
        * 𝔔𝔚𝔈.𝔗𝔜𝔘.𝔒𝔓𝔄𝔖𝔇𝔉𝔊.𝔍𝔎𝔏.𝔛.𝔙𝔅𝔑𝔐 <kbd>⇧</kbd>
        * 𝔮𝔴𝔢𝔯𝔱𝔶𝔲𝔦𝔬𝔭𝔞𝔰𝔡𝔣𝔤𝔥𝔧𝔨𝔩𝔷𝔵𝔠𝔳𝔟𝔫𝔪
    + `en` add <kbd>🕱</kbd>`math𝔣𝖇`, mapped to <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`math𝔣`
        * 𝕼𝖂𝕰𝕽𝕿𝖄𝖀𝕴𝕺𝕻𝕬𝕾𝕯𝕱𝕲𝕳𝕵𝕶𝕷𝖅𝖃𝕮𝖁𝕭𝕹𝕸 <kbd>⇧</kbd>
        * 𝖖𝖜𝖊𝖗𝖙𝖞𝖚𝖎𝖔𝖕𝖆𝖘𝖉𝖋𝖌𝖍𝖏𝖐𝖑𝖟𝖝𝖈𝖛𝖇𝖓𝖒
    + `en` add <kbd>🕱</kbd>`math𝕤`, mapped to <kbd>⎇</kbd><kbd>d</kbd> in <kbd>🕱</kbd>`Math`
        * .𝕎𝔼.𝕋𝕐𝕌𝕀𝕆.𝔸𝕊𝔻𝔽𝔾.𝕁𝕂𝕃.𝕏.𝕍𝔹.𝕄 <kbd>⇧</kbd>
        * ℚℝℙℍℤℂℕ <kbd>⇧</kbd>
        * 𝕢𝕨𝕖𝕣𝕥𝕪𝕦𝕚𝕠𝕡𝕒𝕤𝕕𝕗𝕘𝕙𝕛𝕜𝕝𝕫𝕩𝕔𝕧𝕓𝕟𝕞
        * 𝟘𝟙𝟚𝟛𝟜𝟝𝟞𝟟𝟠𝟡
    + `en` add <kbd>🕱</kbd>`math𝗌`, mapped to <kbd>⎇</kbd><kbd>r</kbd> in <kbd>🕱</kbd>`Math`
        * 𝖰𝖶𝖤𝖱𝖳𝖸𝖴𝖨𝖮𝖯𝖠𝖲𝖣𝖥𝖦𝖧𝖩𝖪𝖫𝖹𝖷𝖢𝖵𝖡𝖭𝖬 <kbd>⇧</kbd>
        * 𝗊𝗐𝖾𝗋𝗍𝗒𝗎𝗂𝗈𝗉𝖺𝗌𝖽𝖿𝗀𝗁𝗃𝗄𝗅𝗓𝗑𝖼𝗏𝖻𝗇𝗆
        * 𝟢𝟣𝟤𝟥𝟦𝟧𝟨𝟩𝟪𝟫
    + `en` add Mathematical Sans-serif Bold <kbd>🕱</kbd>`math𝗯`, mapped to <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`math𝗌`
        * 𝗤𝗪𝗘𝗥𝗧𝗬𝗨𝗜𝗢𝗣𝗔𝗦𝗗𝗙𝗚𝗛𝗝𝗞𝗟𝗭𝗫𝗖𝗩𝗕𝗡𝗠 <kbd>⇧</kbd>
        * 𝗾𝘄𝗲𝗿𝘁𝘆𝘂𝗶𝗼𝗽𝗮𝘀𝗱𝗳𝗴𝗵𝗷𝗸𝗹𝘇𝘅𝗰𝘃𝗯𝗻𝗺
        * 𝟬𝟭𝟮𝟯𝟰𝟱𝟲𝟳𝟴𝟵
    + `en` add Mathematical Sans-serif Italic <kbd>🕱</kbd>`math𝘪`, mapped to <kbd>⎇</kbd><kbd>i</kbd> in <kbd>🕱</kbd>`math𝗌`
        * 𝘘𝘞𝘌𝘙𝘛𝘠𝘜𝘐𝘖𝘗𝘈𝘚𝘋𝘍𝘎𝘏𝘑𝘒𝘓𝘡𝘟𝘊𝘝𝘉𝘕𝘔 <kbd>⇧</kbd>
        * 𝘲𝘸𝘦𝘳𝘵𝘺𝘶𝘪𝘰𝘱𝘢𝘴𝘥𝘧𝘨𝘩𝘫𝘬𝘭𝘻𝘹𝘤𝘷𝘣𝘯𝘮
    + `en` add Mathematical Sans-serif Bold Italic <kbd>🕱</kbd>`math𝙗𝙞`, mapped to <kbd>⎇</kbd><kbd>i</kbd> in <kbd>🕱</kbd>`math𝗯` or <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`math𝘪`
        * 𝙌𝙒𝙀𝙍𝙏𝙔𝙐𝙄𝙊𝙋𝘼𝙎𝘿𝙁𝙂𝙃𝙅𝙆𝙇𝙕𝙓𝘾𝙑𝘽𝙉𝙈 <kbd>⇧</kbd>
        * 𝙦𝙬𝙚𝙧𝙩𝙮𝙪𝙞𝙤𝙥𝙖𝙨𝙙𝙛𝙜𝙝𝙟𝙠𝙡𝙯𝙭𝙘𝙫𝙗𝙣𝙢
    + `en` add Mathematical Monospace <kbd>🕱</kbd>`math𝚖`, mapped to <kbd>⎇</kbd><kbd>m</kbd> in <kbd>🕱</kbd>`Math`
        * 𝚀𝚆𝙴𝚁𝚃𝚈𝚄𝙸𝙾𝙿𝙰𝚂𝙳𝙵𝙶𝙷𝙹𝙺𝙻𝚉𝚇𝙲𝚅𝙱𝙽𝙼 <kbd>⇧</kbd>
        * 𝚚𝚠𝚎𝚛𝚝𝚢𝚞𝚒𝚘𝚙𝚊𝚜𝚍𝚏𝚐𝚑𝚓𝚔𝚕𝚣𝚡𝚌𝚟𝚋𝚗𝚖
        * 𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿
    + `en` add Mathematical Greek Bold <kbd>🕱</kbd>`math𝛃`, mapped to <kbd>⎇</kbd><kbd>g</kbd> in <kbd>🕱</kbd>`Math`
    ...
    + `en` add Mathematical Greek Italic <kbd>🕱</kbd>`math𝜄`, mapped to <kbd>⎇</kbd><kbd>i</kbd> in <kbd>🕱</kbd>`math𝛃`
        * 𝛺𝛦𝛲𝛵𝛶𝛩𝛪𝛰𝛱𝛢𝛴𝛥𝛷𝛤𝛨𝛯𝛫𝛬𝛧𝛸𝛹𝛣𝛮𝛭 <kbd>⇧</kbd>
        * 𝜔𝜀𝜌𝜏𝜐𝜃𝜄𝜊𝜋𝛼𝜎𝛿𝜑𝛾𝜂𝜉𝜅𝜆𝜁𝜒𝜓𝛽𝜈𝜇  𝜍
        * .𝜖𝜚..𝜗..𝜛..𝜕𝜙...𝜘 <kbd>⎇</kbd>
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>u</kbd> 𝛳
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>d</kbd> 𝛻
    + `en` add Mathematical Greek Bold Italic <kbd>🕱</kbd>`math𝜾`, mapped to <kbd>⎇</kbd><kbd>b</kbd> in <kbd>🕱</kbd>`math𝛃`
        * 𝜴𝜠𝜬𝜯𝜰𝜣𝜤𝜪𝜫𝜜𝜮𝜟𝜱𝜞𝜢𝜩𝜥𝜦𝜡𝜲𝜳𝜝𝜨𝜧 <kbd>⇧</kbd>
        * 𝝎𝜺𝝆𝝉𝝊𝜽𝜾𝝄𝝅𝜶𝝈𝜹𝝋𝜸𝜼𝝃𝜿𝝀𝜻𝝌𝝍𝜷𝝂𝝁  𝝇
        * .𝝐𝝔..𝝑..𝝕..𝝏𝝓...𝝒 <kbd>⎇</kbd>
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>u</kbd> 𝜭
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>d</kbd> 𝜵
    + `en` add Mathematical Greek Sans-Serif Bold <kbd>🕱</kbd>`math𝝱`, mapped to <kbd>⎇</kbd><kbd>s</kbd> in <kbd>🕱</kbd>`math𝛃`
        * 𝝮𝝚𝝦𝝩𝝪𝝝𝝞𝝤𝝥𝝖𝝨𝝙𝝫𝝘𝝜𝝣𝝟𝝠𝝛𝝬𝝭𝝗𝝢𝝡 <kbd>⇧</kbd>
        * 𝞈𝝴𝞀𝞃𝞄𝝷𝝸𝝾𝝿𝝰𝞂𝝳𝞅𝝲𝝶𝝽𝝹𝝺𝝵𝞆𝞇𝝱𝝼𝝻  𝞁
        * .𝞊𝞎..𝞋..𝞏..𝞉𝞍...𝞌 <kbd>⎇</kbd>
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>u</kbd> 𝝧
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>d</kbd> 𝝯
    + `en` add Mathematical Greek Sans-Serif Bold Italic <kbd>🕱</kbd>`math𝞲`, mapped to <kbd>⎇</kbd><kbd>w</kbd> in <kbd>🕱</kbd>`math𝛃` or <kbd>⎇</kbd><kbd>i</kbd> in <kbd>🕱</kbd>`math𝝱`
        * 𝞨𝞔𝞠𝞣𝞤𝞗𝞘𝞞𝞟𝞐𝞢𝞓𝞥𝞒𝞖𝞝𝞙𝞚𝞕𝞦𝞧𝞑𝞜𝞛 <kbd>⇧</kbd>
        * 𝟂𝞮𝞺𝞽𝞾𝞱𝞲𝞸𝞹𝞪𝞼𝞭𝞿𝞬𝞰𝞷𝞳𝞴𝞯𝟀𝟁𝞫𝞶𝞵  𝞻
        * .𝟄𝟈..𝟅..𝟉..𝟃𝟇...𝟆 <kbd>⎇</kbd>
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>u</kbd> 𝞡
        * <kbd>⇧</kbd><kbd>⎇</kbd><kbd>d</kbd> 𝞩
    + `en` add Dotless Italic in <kbd>🕱</kbd>`math𝑖`
        * 𝚤𝚥 <kbd>⎇</kbd>
